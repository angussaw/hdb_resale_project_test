"""
## streamlit_app_local.py contains the front logic to load the dashboard that users
will interact with and input details of a hdb flat to generate predictions

This local version is for testing purposes. fast_api.py is run before running this file
"""
    ...
    if submit:
        ...
        if len(messages) > 0:
            ...
            derived_input_data = requests.post(
                url="http://localhost:8500/dataprep", data=json.dumps(input, default=str)
            )
            ...
            with right:
                ...
                shap_values = requests.post(
                    url="http://localhost:8500/explain",
                    data=json.dumps(derived_input_data.json(), default=str),
                )
                if shap_values.json():
                    # Render waterfall plot of shap values to explain model prediction
