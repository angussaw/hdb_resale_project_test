...
class FeatureEngineer:
    ...
    def __init__(
        self, params: dict, inference_mode: bool = False, directory=None
    ) -> None:
    ...
    def engineer_features(self, hdb_data: pd.DataFrame) -> pd.DataFrame:
        ...
        hdb_data = self.calculate_lease_age(
            hdb_data, self.feature_engineering_params["calculate_lease_age"]
        )

        logger.info("Generating amenity features...")
        amenity_features_list = self.generate_amenities_features(
            hdb_data, self.feature_engineering_params["generate_amenities_features"]
        )
    ...
    def map_regions(self, hdb_data: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        hdb_data[region_feature] = hdb_data[town_feature].map(
            {town: region for region, towns in map_regions.items() for town in towns}
        )
    ...
    def calculate_lease_age(self, hdb_data: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        hdb_data[lease_age_feature] = (
            hdb_data[self.year_feature] - hdb_data[lease_commence_date_feature]
        )
    ...
    def generate_amenities_features(self, hdb_data: pd.DataFrame, params: dict) -> list:
        ...
        for amenity in amenities:
            ...
            feature_df = self.get_nearests_amenities(
                pd.concat([hdb_coordinates, hdb_data[self.year_month_feature]], axis=1),
                amenity,
                latitude_feature,
                longitude_feature,
                **amenities[amenity],
    ...
    def get_nearests_amenities(
        ...
        if not self.inference_mode:
            ...
            columns = [
                no_of_amenities_within_radius,
                distance_to_nearest_amenity,
                f"nearest_{amenity}_coordinates",
                f"nearest_{amenity}_name",
            ]

        amenity_features = pd.DataFrame(
            hdb_coordinates.progress_apply(
                lambda coordinates: hdb_est.utils.find_nearest_amenities(
                    coordinates,
                    amenity_details=amenity_details,
                    radius=radius,
                    period=period,
