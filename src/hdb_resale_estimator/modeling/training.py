...
def train_pipeline(
    ...
    Instantiates a create a ClassicalModelBuilder object and sets model parameters specified in the Hydra config file
    ...
    Returns the model's performance on the specified optimization metric (eg root mean square error)
    and the model uri
    ...
    if label_column in feature_label_data.columns:
        ...
        features = feature_label_data[
            [col for col in feature_label_data.columns if col != label_column]
        ]
    else:
    ...
    if config["process_train_data"]["ordinal_encoding"]:
        ...
        features = builder._ordinal_encode_variables(
            feature_data=features,
            columns=config["process_train_data"]["ordinal_encoding"],
        )
    ...
    if config["model_params"][chosen_model]["scale_data"]:
        ...
        datasets["train"]["X"] = builder.scale_data(
            datasets["train"]["X"].sort_index(axis=1), fitted_scaler=None
        )
        datasets["test"]["X"] = builder.scale_data(
            datasets["test"]["X"].sort_index(axis=1),
            fitted_scaler=builder.objects["standard_scaler"]["scaler"],
        )
        try:
            datasets["val"]["X"] = builder.scale_data(
                datasets["val"]["X"].sort_index(axis=1),
                fitted_scaler=builder.objects["standard_scaler"]["scaler"],
            )
        except:
            pass
    ...
    metrics, visualizations_save_dir = evaluator.evaluate_model(datasets=datasets)
