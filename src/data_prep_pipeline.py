...
def main():
    with hdb_est.utils.timer("Data Preparation"):
        ...
        logger.info("Saving derived hdb features to %s...", data_prep_config["files"]["training"]["save_to_source"])
        if data_prep_config["files"]["training"]["save_to_source"] == "postgres":
            db_engine = hdb_est.utils.create_postgres_engine()
            derived_features_table_name = data_prep_config["files"]["training"]["derived_features_table_name"]
            hdb_est.utils.push_data_to_sql(db_engine, hdb_preprocessed, derived_features_table_name)
            logger.info("Derived mppa features is saved in %s", derived_features_table_name)

        elif data_prep_config["files"]["training"]["save_to_source"] == "local":
            preprocessed_save_path = data_prep_config["files"]["training"]["preprocessed_save_path"]
