...
def main():
    ...
    st.set_page_config(layout="wide", page_title="HDB Resale Price Estimator", page_icon=Image.open("images/house.png"))
    ...
    st.caption('It also generates nearby amenities of the flat, and uses it to estimate the resale price')
    st.caption('Please input the flat details on the left sidebar and click "Estimate resale price" to generate predictions!')
    with st.sidebar:
    ...
    if submit:
        ...
        if len(messages) > 0:
            ...
            input_df = input_df[[col for col in input_df.columns if col not in ["id","remaining_lease"]]]
            ...
            distance_to_nearest_amenity_df = derived_input_data_df[[col for col in derived_input_data_df.columns if col.startswith("distance_to_nearest")]]
            distance_to_nearest_amenity_df.columns = [col.replace("distance_to_nearest_","").upper() for col in distance_to_nearest_amenity_df.columns]
            ...
            flat_address = derived_input_data_df.iloc[0]["block"] + " " + derived_input_data_df.iloc[0]["street_name"]
            ...
            nearest_amenity_coordinates_col = derived_input_data_df.columns[derived_input_data_df.columns.str.endswith("coordinates")]
